...
use App\Helper\Restaurant\OrderHelper;
...
class OrderController extends Controller
{
    public function index()
    {
        return view('restaurant_owner.orders-list', [
            ...
            'orders' => OrderHelper::getThisRestaurantNotDeliveredOrders()
        ]);
    }
    ...
    private function calculateTotalIncome()
    {
        ...
        $orders = OrderHelper::getThisRestaurantOrders();

        $totalIncome = $orders->reduce($income,
            fn($order) =>
                $income += $order->cart->total_price,
            0);
